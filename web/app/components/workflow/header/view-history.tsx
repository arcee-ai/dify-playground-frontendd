...
const ViewHistory = ({ withText }: ViewHistoryProps) => {
  ...
  const showWorkflowruns = appDetail && !isChatMode && open
  const { data: runList, isLoading: runListLoading } = useSWR(
    showWorkflowruns ? `/apps/${appDetail.id}/workflow-runs` : null,
    fetchWorkflowRunHistory,
  )
  const showChatRunHistory = appDetail && isChatMode && open
  ...
  return (
    ...
    >
        ...
        {withText && (
          <div
            className={cn(
              ...
              open ? 'btn-accent-ghost' : 'btn-ghost',
            )}
          >
            <Aicon size={20} icon="icon-play-history" className="a-icon--btn" />
        ...
        {!withText && (
          <Tooltip popupContent={t('workflow.common.viewRunHistory')}>
            <div
              className={cn(
                'btn btn-medium btn-icon',
                open ? 'btn-ghost-accent' : 'btn-ghost',
              )}
              onClick={() => {
                setCurrentLogItem()
      ...
      <PortalToFollowElemContent className="z-[12]">
        <div
          className="flex flex-col ml-2 w-[240px] fade-up-8 card-m overflow-y-auto"
          style={{
