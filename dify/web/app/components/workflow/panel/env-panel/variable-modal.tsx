...
const VariableModal = ({ env, onClose, onSave }: ModalPropsType) => {
  ...
  const [type, setType] = React.useState<'string' | 'number' | 'secret'>(
    'string',
  )
  ...
  return (
    <div
      className={cn(
        'fade flex flex-col w-[360px] card-m h-full p-4 shadow-2xl',
      )}
    >
      <div className="shrink-0 flex items-center justify-between mb-3 text-text-primary system-xl-semibold">
        {!env
          ? t('workflow.env.modal.title')
          : t('workflow.env.modal.editTitle')}
      ...
      <div className="py-2">
        {/* type */}
        <div className="mb-4">
          <div className="mb-1 h-6 flex items-center text-text-secondary system-sm-semibold">
          ...
          <div className="flex gap-2">
            <div
              className={cn(
                ...
                type === 'string'
                  && 'text-text-primary system-sm-medium border-[1.5px] shadow-xs bg-components-option-card-option-selected-bg border-components-option-card-option-selected-border hover:border-components-option-card-option-selected-border',
              )}
            ...
            <div
              className={cn(
                'w-[106px] flex items-center justify-center p-2 radius-md bg-components-option-card-option-bg border border-components-option-card-option-border text-text-secondary system-sm-regular cursor-pointer hover:shadow-xs hover:bg-components-option-card-option-bg-hover hover:border-components-option-card-option-border-hover',
                type === 'number'
                  && 'text-text-primary font-medium border-[1.5px] shadow-xs bg-components-option-card-option-selected-bg border-components-option-card-option-selected-border hover:border-components-option-card-option-selected-border',
              )}
              onClick={() => {
                setType('number')
                if (!/^[0-9]$/.test(value))
      ...
      <div className="pt-4 flex flex-row-reverse rounded-b-2xl">
        <div className="flex gap-2">
