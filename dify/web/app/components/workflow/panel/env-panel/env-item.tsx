...
const EnvItem = ({ env, onEdit, onDelete }: EnvItemProps) => {
  ...
  return (
    <div
      className={cn(
        'mb-1 px-2 py-2 bg-components-panel-on-panel-item-bg radius-md border border-gray-200 hover:bg-components-panel-on-panel-item-bg-hover transition duration-300',
        destructive
      ...
      <div className="flex items-center justify-between">
        <div className="grow flex gap-1 items-center">
          ...
          {env.value_type === 'secret' && (
            <RiLock2Line className="w-3 h-3 text-text-tertiary" />
          )}
        </div>
